...
class ConstraintsResolver:
    def __init__(self, reader, device='cuda', polygon=False, room_conflict=False):
        self.reader = reader
        self.device = device
        self.polygon = polygon
        self.room_conflict = True
        self.num_c = len(self.reader.classes)
        self.x = {}  # x[cid, time_idx, room_id]: 课程-时间-教室 -> index of x_tensor 
        self.xidx_to_x = {} # xidx_to_x[index of x_tensor]: 课程-时间-教室 -> (cid, time_idx, room_id)
    ...
    def _create_variables(self):
        ...
        room_transformer = []
        week_transformer = [[] for w in range(self.reader.nrWeeks)]
        day_transformer = [[] for w in range(self.reader.nrDays)]
        start_transformer = []
        end_transformer = []
        ...
        for cid in self.reader.classes.keys():
            ...
            for topt, tidx in time_options:
                week_bits, day_bits, start, end = topt["optional_time_bits"]
                time_mat = topt.get('optional_time')
                time_bits = (week_bits, day_bits, start, end)
                for rid in room_options:
                    # 检查这个时间-教室组合是否可用
                    if rid != 'dummy' and not self._is_room_available(rid, time_mat=time_mat, time_bits=time_bits):
                        # 跳过不可用的组合
                    ...
                    self.xidx_to_x[var_count] = (cid, tidx, rid)
                    if self.y.get((cid, tidx), None) == None: self.y[cid, tidx] = []
                    self.y[cid, tidx].append(var_count)
                    if self.w.get((cid, rid), None) == None: self.w[cid, rid] = []
                    self.w[cid, rid].append(var_count)
                    if rid == 'dummy':
                        room_transformer.append(0)
                    else:
                        room_transformer.append(int(rid))
                    for w in range(self.reader.nrWeeks):
                        week_transformer[w].append(int(week_bits[w]))
                    for d in range(self.reader.nrDays):
                        day_transformer[d].append(int(day_bits[d]))
                    start_transformer.append(start)
                    end_transformer.append(end)
                    var_count += 1

        print(f"Filtered {filtered_count} varibles. Created {class_count + var_count} varibles")

        self.x_tensor = torch.zeros(var_count, device=self.device)
        print(f"Create x tensor shape: {self.x_tensor.shape}")
        self.room_transformer = torch.tensor(room_transformer, device=self.device)
        print(f"Create room transformer shape: {self.room_transformer.shape}")
        self.week_transformer = torch.tensor(week_transformer, device=self.device)
        print(f"Create week transformer shape: {self.week_transformer.shape}")
        self.day_transformer = torch.tensor(day_transformer, device=self.device)
        print(f"Create day transformer shape: {self.day_transformer.shape}")
        self.start_transformer = torch.tensor(start_transformer, device=self.device)
        print(f"Create start transformer shape: {self.start_transformer.shape}")
        self.end_transformer = torch.tensor(end_transformer, device=self.device)
        print(f"Create end transformer shape: {self.end_transformer.shape}")

        # Bitmask representations for fast overlap detection (int64, CPU)
        week_pow = (1 << torch.arange(self.reader.nrWeeks)).long()   # [nrWeeks]
        day_pow  = (1 << torch.arange(self.reader.nrDays)).long()    # [nrDays]
        self.weeks_int_transformer = (self.week_transformer.cpu().long() * week_pow.unsqueeze(1)).sum(0)
        self.days_int_transformer  = (self.day_transformer.cpu().long() * day_pow.unsqueeze(1)).sum(0)
    ...
    def _add_primary_constraints(self):
        ...
        for cid in self.reader.classes.keys():
            ...
            if valid_x_vars:
                self.valid_solution_constraint[f"assign_{cid}"] = (valid_x_vars, 1)
        self.valid_dist_tensor = self.build_valid_dist()
    ...
    def _add_single_distribution_constraint(self, constraint, is_hard):
        ...
        ##############################################################################################
            ...
            if disploygon: self.mask_constraints.append((constraint, is_hard))
            ...
            # Parse MaxBlock(M,S) format
            # Don't skip - compute directly for actual solution evaluation
            params = ctype.split('(')[1].rstrip(')').split(',')
            max_block_length = int(params[0])
            max_gap_in_block = int(params[1])
            self._add_max_block_constraint(classes, max_block_length, max_gap_in_block, is_hard, penalty, False)
        else:
    ...
    def _set_objective(self):
        ...
        # 目标1：torch.sum(torch.matmul(x.view(1, -1), w_valid.view(-1, 1))) == self.num_c

        # w_valid = self.valid_dist_tensor
        ...
        # 1. 时间惩罚 (raw penalty, weight applied in evaluator)
            ...
            for topt, tidx in time_opts:
                ...
                if penalty > 0:
                    if self.y.get((cid, tidx), None):
                        w_time[self.y[cid, tidx]] += penalty

        # 2. 教室惩罚 (raw penalty, weight applied in evaluator)
        ...
        for cid in self.reader.classes.keys():
            ...
            for ropt in class_data['room_options']:
                ...
                if penalty > 0 and (cid, rid) in self.w:
                    indices = self.w.get((cid, rid), [])
                    if indices:
                        w_room[indices] += penalty
        
        # 3. 分布约束惩罚 (raw penalty, weight applied in evaluator)
        ...
        soft_dist_cost = self.soft_dist_cost_tensor # shape (1, soft_dist length)

        self.objective_penalty = (w_time, w_room, (soft_dist, soft_upper_dist, soft_dist_cost), 
                                   (time_weight, room_weight, dist_weight))
                        ...
                        for r1 in room_opts1:
                            for r2 in room_opts2:
                                ...
                                if (start1 + end1 + travel1 <= start2) or (start2 + end2 + travel2 <= start1) or (and_days == 0) or (and_week == 0):
                                    ...
                                    if (c1, tidx1, r1) not in self.x or (c2, tidx2, r2) not in self.x:
                                        continue
                                    x_index = [self.x[c1, tidx1, r1], self.x[c2, tidx2, r2]]
                                    if is_hard:
                                        if self.hard_constraints_cache.get(f"y_{c1}_{tidx1}_{c2}_{tidx2}_1", False) == True: continue
                                        else: self.hard_constraints_cache[f"y_{c1}_{tidx1}_{c2}_{tidx2}_1"] = True
    ...
    def _add_max_block_constraint(self, classes, max_block_length, max_gap_in_block, is_hard, penalty, afterAssign):
        ...
        Following constraints.py logic:
        - Count violations across all weeks/days
        - Return sum(overM_blocks) / nrWeeks
        """
        # Process MaxBlock constraint (removed early return - compute directly)
        ...
        for cid in classes:
            ...
            for topt, tidx in self.class_to_time_options[cid]:
                if self.y.get((cid, tidx), None):
                    all_class_vars.extend(self.y[cid, tidx])
        
        if len(all_class_vars) < 2:
            return
        
        all_class_vars = sorted(set(all_class_vars))
        
        overM_blocks = 0
        for w in range(self.reader.nrWeeks):
            for d in range(self.reader.nrDays):
                day_vars = []
                for xidx in all_class_vars:
                    if self.week_transformer[w, xidx].item() == 1 and self.day_transformer[d, xidx].item() == 1:
                        day_vars.append(xidx)
                
                if len(day_vars) < 2:
                    continue
                
                day_vars_sorted = sorted(day_vars, key=lambda x: self.start_transformer[x].item())
                
                block_len = 0
                prev_end = 0
                first = True
                
                for xidx in day_vars_sorted:
                    start = self.start_transformer[xidx].item()
                    end = start + self.end_transformer[xidx].item()
                    gap = start - prev_end
                    
                    if first:
                        block_len = 1
                        first = False
                    elif gap <= max_gap_in_block:
                        block_len += 1
                    else:
                        block_len = 1
        ...
        avg_viol = int(overM_blocks / max(self.reader.nrWeeks, 1))
        if avg_viol >= 0 and penalty is not None:
            constraint_name = f"MaxBlock_{max_block_length}_{max_gap_in_block}"
            self.soft_constraints[constraint_name] = ([], 0, penalty * avg_viol)
    ...
    def _add_room_capacity_constraints(self):
        """
        向量化版本：预先计算所有可能的冲突约束
        使用全1张量来模拟所有变量都可能被选中的情况
        """
        print("Vectorized constraint generation (all variables considered)...")
        
        # 创建一个全1的虚拟x_tensor用于约束生成
        x_pseudo = torch.ones_like(self.x_tensor)
        
        constraint_count = 0
        all_constraint_pairs = []  # 存储所有约束对 (xidx1, xidx2)
        
        # 对每个真实房间
        unique_rooms = torch.unique(self.room_transformer)
        
        for room_id in tqdm(unique_rooms, desc="Processing rooms"):
            if room_id == 0:  # 跳过 dummy room
                continue
            
            # 找到此房间的所有变量
            room_mask = self.room_transformer == room_id
            room_var_indices = torch.where(room_mask)[0]
            
            if len(room_var_indices) <= 1:
                continue
            
            n_room = len(room_var_indices)

            # All computations on CPU; chunked to cap peak RAM at ~256 MB/room
            CHUNK     = 512
            idx_cpu   = room_var_indices.cpu()
            rs        = self.start_transformer.cpu()[idx_cpu]   # [n_room]
            re        = rs + self.end_transformer.cpu()[idx_cpu]
            rw_int    = self.weeks_int_transformer[idx_cpu]     # [n_room] int64
            rd_int    = self.days_int_transformer[idx_cpu]      # [n_room] int64

            conflict_i_list: list = []
            conflict_j_list: list = []

            for row_start in range(0, n_room, CHUNK):
                row_end = min(row_start + CHUNK, n_room)
                rs_i  = rs[row_start:row_end].unsqueeze(1)   # [chunk, 1]
                re_i  = re[row_start:row_end].unsqueeze(1)
                rw_i  = rw_int[row_start:row_end].unsqueeze(1)
                rd_i  = rd_int[row_start:row_end].unsqueeze(1)

                # only compare against j > i (upper-triangle pairs)
                for col_start in range(row_start, n_room, CHUNK):
                    col_end = min(col_start + CHUNK, n_room)
                    rs_j  = rs[col_start:col_end].unsqueeze(0)   # [1, chunk]
                    re_j  = re[col_start:col_end].unsqueeze(0)
                    rw_j  = rw_int[col_start:col_end].unsqueeze(0)
                    rd_j  = rd_int[col_start:col_end].unsqueeze(0)

                    overlap = (
                        (rs_i < re_j) & (rs_j < re_i) &
                        ((rw_i & rw_j) != 0) &
                        ((rd_i & rd_j) != 0)
                    )  # [chunk_row, chunk_col] bool

                    # zero out lower-triangle within same diagonal block
                    if row_start == col_start:
                        diag = torch.arange(row_end - row_start)
                        overlap[diag, diag] = False
                        tril_idx = torch.tril_indices(row_end - row_start,
                                                      col_end - col_start,
                                                      offset=-1)
                        overlap[tril_idx[0], tril_idx[1]] = False

                    pairs = torch.where(overlap)
                    conflict_i_list.extend((pairs[0] + row_start).tolist())
                    conflict_j_list.extend((pairs[1] + col_start).tolist())

            for i, j in zip(conflict_i_list, conflict_j_list):
                xidx1 = idx_cpu[i].item()
                xidx2 = idx_cpu[j].item()
                
                # 检查是否已经添加过这个约束（使用缓存）
                cache_key = f"x_{xidx1}_{xidx2}_room_capacity"
                if self.hard_constraints_cache.get(cache_key, False):
                    continue
                
                self.hard_constraints_cache[cache_key] = True
                
                # 添加约束：x[xidx1] + x[xidx2] <= 1
                # 这表示这两个变量不能同时为1（不能同时占用该房间的重叠时间）
                constraint_name = f"SameRoomCrash_{xidx1}_{xidx2}"
                self.hard_constraints[constraint_name] = ([xidx1, xidx2], 1)
                
                all_constraint_pairs.append((xidx1, xidx2))
                constraint_count += 1
        print(f"Added {constraint_count} room capacity constraints")
        return all_constraint_pairs, []
    ...
    #     if afterAssign:
    #         """
    #         检查被安排到同一房间的课程是否有时间重叠
            
    #         Args:
    #             x_values: 如果提供，使用此值；否则使用 self.x_tensor
            
    #         Returns:
    #             violations: list of tuples (xidx1, xidx2) 表示违反约束的变量对
    #         """
    #         x_values = self.x_tensor.clone()
    #         # 只考虑被赋值为1的变量
    #         assigned_mask = x_values > 0.5  # 使用0.5作为阈值处理浮点数
    ...
    #         # 获取被赋值变量的属性
    #         assigned_rooms = self.room_transformer[assigned_indices]
    #         assigned_weeks = self.week_transformer[:, assigned_indices]  # [nrWeeks, n_assigned]
    #         assigned_days = self.day_transformer[:, assigned_indices]    # [nrDays, n_assigned]
    #         assigned_starts = self.start_transformer[assigned_indices]
    #         assigned_ends = self.end_transformer[assigned_indices]

    #         n_assigned = len(assigned_indices)
    #         violations = []

    #         # 对每个真实房间（非dummy）进行检查
    #         unique_rooms = torch.unique(assigned_rooms)
            
    #         for room_id in unique_rooms:
    #             if room_id == 0:  # 跳过 dummy room
    #                 continue
    #             # 找到分配到此房间的所有变量
    #             room_mask = assigned_rooms == room_id
    #             room_indices_local = torch.where(room_mask)[0]  # 在assigned_indices中的局部索引
                
    #             if len(room_indices_local) <= 1:
    #                 continue

    #             # 获取这些变量的时间信息
    #             room_weeks = assigned_weeks[:, room_indices_local]  # [nrWeeks, n_room]
    #             room_days = assigned_days[:, room_indices_local]    # [nrDays, n_room]
    #             room_starts = assigned_starts[room_indices_local]
    #             room_ends = assigned_ends[room_indices_local]

    #             n_room = len(room_indices_local)

    #             # 计算实际结束时间 (start + duration)
    #             actual_ends = room_starts + room_ends

    #             # 两两比较时间重叠
    #             # 使用广播创建成对比较
    #             starts_i = room_starts.unsqueeze(1)  # [n_room, 1]
    #             ends_i = actual_ends.unsqueeze(1)    # [n_room, 1]
    #             starts_j = room_starts.unsqueeze(0)  # [1, n_room]
    #             ends_j = actual_ends.unsqueeze(0)    # [1, n_room]

    #             # 检查时间段重叠: [start_i, end_i) 和 [start_j, end_j) 重叠
    #             # 重叠条件: start_i < end_j AND start_j < end_i
    #             time_overlap = (starts_i < ends_j) & (starts_j < ends_i)  # [n_room, n_room]

    #             # 检查周重叠 (至少有一周相同)
    #             # weeks_i = room_weeks.T.unsqueeze(2)  # [n_room, nrWeeks, 1]
    #             # weeks_j = room_weeks.T.unsqueeze(0)  # [1, nrWeeks, n_room]
    #             # week_overlap = (weeks_i & weeks_j).any(dim=1)  # [n_room, n_room]
    #             room_weeks_t = room_weeks.T  # [n_room, nrWeeks]
    #             weeks_i = room_weeks_t.unsqueeze(1)  # [n_room, 1, nrWeeks]
    #             weeks_j = room_weeks_t.unsqueeze(0)  # [1, n_room, nrWeeks]
    #             week_overlap = (weeks_i & weeks_j).any(dim=2)  # [n_room, n_room]

    #             # 检查日重叠 (至少有一天相同)
    #             room_days_t = room_days.T  # [n_room, nrDays]
    #             days_i = room_days_t.unsqueeze(1)    # [n_room, 1, nrDays]
    #             days_j = room_days_t.unsqueeze(0)    # [1, n_room, nrDays]
    #             day_overlap = (days_i & days_j).any(dim=2)  # [n_room, n_room]
                

    #             # 综合判断: 时间、周、日都重叠才算冲突
    #             overlap_mask = time_overlap & week_overlap & day_overlap

    #             # 只保留上三角部分，避免重复 (i < j)
    #             triu_mask = torch.triu(torch.ones_like(overlap_mask, dtype=torch.bool), diagonal=1)
    #             overlap_mask = overlap_mask & triu_mask

    #             # 找到所有冲突对
    #             conflict_pairs = torch.where(overlap_mask)

    #             for i, j in zip(conflict_pairs[0].tolist(), conflict_pairs[1].tolist()):
    #                 xidx1 = assigned_indices[room_indices_local[i]].item()
    #                 xidx2 = assigned_indices[room_indices_local[j]].item()
    #                 violations.append((xidx1, xidx2))
    ...
    def build_valid_dist(self):
        num_c = len(self.valid_solution_constraint)
        ...
        for i, (key, (x_index, u)) in enumerate(self.valid_solution_constraint.items()):
            for idx in x_index:
